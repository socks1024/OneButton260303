...
## 将InputEvent转换为用户友好的字符串表示，适用于UI显示（没有鼠标输入）
...
## 判断一个 InputAction 是不是内置的 UI Action
static func not_internal_ui_action(action_name: StringName) -> bool:
	return !action_name.contains("ui_")
